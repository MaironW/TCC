...
void EulerAngles(){
    yaw = atan2f(2*(q1*q2 + q0*q3), 1 - 2*(q2*q2 + q3*q3)) * 180.0/PI;
    pitch = asinf(2*(q0*q2 - q1*q3)) * 180.0/PI;
    roll = atan2f(2*q2*q3 + 2*q0*q1, 1 - 2*(q1*q1+q2*q2)) * 180.0/PI;
}
...
void NEDAcceleration(){
    ...
    // aNED = q*a*q'
    QuaternionProduct(prod, q, v);      // q*a
    QuaternionProduct(prod, prod,qj);   // (q*a)*q'
...
void LinearAcceleration(float acclX,float acclY,float acclZ){
    ...
    acclXlin = acclX - gravX*GRAVITY;
    acclYlin = acclY - gravY*GRAVITY;
    acclZlin = acclZ - gravZ*GRAVITY;
}

// calculate directions for gravity based on current quaternions in G
void GravityVector(){
    // float prod[4] = {0,0,0,0};
    ...
    //
    // // g_sensor = q'*g*q
    // QuartenionProduct(prod, qj, v);      // q'*a
    // QuartenionProduct(prod, prod,q);   // (q'*a)*q
    //
    // gravX = prod[1];
    // gravY = prod[2];
    // gravZ = prod[3];
...
void DeclinationAlign(){
    double decl = -MAGNETIC_DECLINATION*PI/360.0; // [rad] declination/2 
    float qDecl[4] = {0,0,0,0};

    qDecl[0] = q0*cos(decl)-q3*sin(decl);
    qDecl[1] = q1*cos(decl)+q2*sin(decl);
    qDecl[2] = -q1*sin(decl)+q2*cos(decl);
    qDecl[3] = q0*sin(decl)+q3*cos(decl);
